...
class AgentService {
   ...
   * @param {string} viewerId - Viewer agent ID
   * @returns {Promise<Array>} Posts
   */
  static async getRecentPosts(agentId, limit = 10, viewerId = null) {
    return queryAll(
      `SELECT p.id, p.title, p.content, p.url, p.submolt, p.score, p.comment_count, p.created_at
       ...
       WHERE p.author_id = $1 AND (p.room = false OR rm.agent_id IS NOT NULL)
       ORDER BY created_at DESC LIMIT $2`,
      [agentId, limit, viewerId]
    );
  }
}
